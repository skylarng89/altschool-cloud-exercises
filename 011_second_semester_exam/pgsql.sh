...
# Create User
sudo -i -u postgres psql -c "CREATE USER laraveluser WITH ENCRYPTED PASSWORD 'laraveluser'"

# Grant user privilege on database
sudo -i -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE laraveldb to laraveluser"

# Configure user login method in pg_hba.conf
echo -e 'local\tall\t\tlaraveluser\t\t\t\t\tmd5' >> /etc/postgresql/15/main/pg_hba.conf
